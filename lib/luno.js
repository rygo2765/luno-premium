...
export async function fetchLuno() {
    try {
    ...
    } catch (err) { //use catch (err) to catch abnormal or unexpected cases i.e. response.status === 200 but BTCMYR['bid'] doesn't return value
        if (err == "Fetch failed") {
